resource "databricks_job" "batch_inference_job" {
  name = "${local.env_prefix}mlops-jetstar-batch-inference-job"
  ...
  notebook_task {
    ...
    base_parameters = {
      ...
      input_table_name  = "jetstar_feature_store_taxi_example.inference_data"
      output_table_name = "jetstar_feature_store_taxi_example.mlopsjetstar_predictions"
    }
  }
